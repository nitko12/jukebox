...
async function refreshQueue(data) {
    ...
    data.forEach(el => {
        ...
        <tr>
            <td class="trend-br1"><iframe src="https://www.youtube-nocookie.com/embed/${el.url}" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></td>
            <td class="trend-broj-1"><p class="trend-broj">#${cnt++}</p></td>
            <td class="naziv-pjesme"><h4 id="naziv-pjesme">Odabralo: ${el.votes}</h4></td>
                    <td class="trend-broj-${el.votes}"><p class="trend-broj"><i class="material-icons glasaj-icon" style="font-size:70px;" title="Daj glas ovoj pjesmi" id="glasaj" onclick="vote('${el.id}')">thumb_up</i></p></td>
        </tr>
    </table>`;
